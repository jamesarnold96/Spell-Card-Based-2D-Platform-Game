...
public class CharacterController2D : MonoBehaviour
{
    ...
    {
        if (Parameters.JumpRestrictions == ControllerParameters2D.JumpBehaviour.CanJumpOnGround)
            State.IsGrounded = false; // trial soln
    ...
    private void CalculateRayOrigins()
    {
        ...
        var centre = new Vector2(_boxCollider.offset.x * _localScale.x, _boxCollider.offset.y * _localScale.y);

        _raycastTopLeft = _transform.position + new Vector3(centre.x - size.x + SkinWidth, centre.y + size.y - SkinWidth);
        _raycastBottomRight = _transform.position + new Vector3(centre.x + size.x - SkinWidth, centre.y - size.y + SkinWidth);
        _raycastBottomLeft = _transform.position + new Vector3(centre.x - size.x + SkinWidth, centre.y - size.y + SkinWidth);
    }
    ...
    private void MoveVertically(ref Vector2 deltaMovement)
    {
        ...
        for (var i = 0; i < TotalVerticalRays; i++)
        {
            ...
            else
            {
                ...
                State.IsGrounded = true; //trial soln
            }
